...
get-adgroupmember -identity "$group" -recursive | Get-Aduser -properties mail,employeeNumber,department,Title,employeeType | select name,mail,employeeNumber,department,Title,employeeType  | export-csv "C:\Scripts\$CSVName.csv" -NoTypeInformation
